...
- Recap of initial status: ✅
- Finalize our methods and bridge the gap and : ✅
...
#### 4.1 Baseline methods: Signed activation quantization -> unsigned activation quantization for all Conv2d layers due to ReLU
...
#### 4.2 Our methods: Replace original model's all Conv2d layers with our layers (including Conv2d in both "_backbone_2d_" and "_dense_head_")
...
#### 4.3 Our methods: Replace original model's backbone_2d's Conv2d layers with our layers and all its dense_head's Conv2d with baseline methods (standard W8A8))
...
#### 4.4 Our methods: with different scaling factor
...
#### 4.5 Test: Only quantizing the Conv2d in dense_head (standard W8A8) without quantizing the Conv2d in backbone_2d (W16A16)
...
#### 4.6 Compare the dense_head activation L1loss between (the original model and the standard W8A8-quantized Conv2d in backbone_2d and our method's Conv2d in dense_head) and (the original model and fully standard W8A8-quantized model) to get the selective list.
...
#### 4.7 Two more experiments as compared to the No.5 test.
...
#### 4.8 Baseline methods: Only quantize Conv2d in backbone_2d with quantizer of Conv2d in dense_head set to 16 num_bits

| Model                                | mAP                             | NDS                  | Structure                                                                                                                                                                           |
| ------------------------------------ | ------------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Original model                       | (0.5921 - 0.5922)               | 0.6648               | No change                                                                                                                                                                           |
| Baseline methods (W8A8 quantization) | ~~(0.5882 - 0.5888)~~ -> 0.5903 | ~~0.6603~~ -> 0.6626 | Only quantize Conv2d in **_backbone_2d_** with quantizer of Conv2d in dense_head set to 16 num_bits                                                                                 |
...
#### 4.9 Our methods: Replace all original model's **_backbone_2d_**'s Conv2d layers and **_dense_head_**'s Conv2d layers with our layers with scaling factor = 0.5, except for the Conv2d in the above list.

| Model                                | mAP                  | NDS                  | Structure                                                                                                                                                                                                                      |
| ------------------------------------ | -------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Original model                       | (0.5921 - 0.5922)    | 0.6648               | No change                                                                                                                                                                                                                      |
| Baseline methods (W8A8 quantization) | 0.5903               | 0.6626               | Only quantize Conv2d in **_backbone_2d_** with quantizer of Conv2d in dense_head set to 16 num_bits                                                                                                                            |
...
#### 4.10 Our methods: Delete the W16A16 Conv2d quantizer for those which are in the above list.
